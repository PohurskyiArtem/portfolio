...
if(document.querySelector('.catalog')) {
  ...
  vars.$catalogFilterItems.forEach(el => {
    el.querySelector('input').addEventListener('change', e => {
      ...
      if(checked) {
        ...
        el.closest(".catalog-filter__bottom").closest(".catalog-filter").querySelector(".catalog-filter__quantity").textContent++;
  ...
  vars.$catalogChoice.addEventListener('click', e => {
    if(e.target.classList.contains('catalog-choice__item')) {
      ...
      console.log(text)
      console.log(document.querySelector(`[data-text="${text}"]`))
      ...
      document.querySelector(`[data-text="${text}"]`).closest(".catalog-filter__bottom").closest(".catalog-filter").querySelector(".catalog-filter__quantity").textContent--;
    }
    ...
    if(e.target.classList.contains('catalog-choice__clear')) {
      Array.from(e.currentTarget.children).forEach(el => {
        ...
        vars.$allFilterQuantities.forEach(el => {
          el.textContent = 0;
        })
      })
